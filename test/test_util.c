...
// // Check that a socket address created with address and port 0 has those
// values. Test(socket_address, zero_addr_and_port) {
//   struct sockaddr_in addr = socket_address(0, 0);
//   cr_assert(zero(u32, addr.sin_addr.s_addr));
//   cr_assert(zero(u16, addr.sin_port));
// }

// // Check a socket address converts its address bytes from host to network
// order. Test(socket_address, addr_byte_order) {
